...
class CR_VAE(nn.Module):
    def __init__(self, dims, n_centroids,bn=False, dropout=0, binary=True):
        super(CR_VAE, self).__init__(dims)
    ...
    def init_gmm_params(self, dataloader, device='cpu'):
        """
        Init CR_VAE model with GMM model parameters
        """
        ...
        with trange(max_iter, disable=verbose) as pbar:
            while True:
                ...
                for input_file_name,expected_file_name in files:
                    ...
                    loader_params = {'batch_size': batch_size, 'shuffle': True,'num_workers': 8, 'drop_last': True, 'pin_memory': True}
